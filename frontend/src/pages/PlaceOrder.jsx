...
function PlaceOrder() {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault()
    if (!formData.firstName || !formData.lastName || !formData.phone || !formData.zipcode || !formData.city || !formData.email || !formData.state || !formData.street) {
    ...
    try {
      ...
      switch(method) {
        ...
        case 'stripe':
          ...
          break;
        case 'razorpay':
          const resposeRazorpay = await axios.post(backendUrl + '/api/order/razorpay', orderData, {headers: {token}});
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            <div onClick={() => setMethod('stripe')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
               ...
               <img src={assets.stripe_logo} alt="Stipe Payment" className='h-5 mx-4' />
            </div>
            <div onClick={() => setMethod('razorpay')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
               <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-400 border-green-400' : ''}`}></p>
               <img src={assets.razorpay_logo} alt="Razorpay Payment" className='h-5 mx-4' />
            </div>
